...
export const postScrapFolder = async (name: string, type: string, contentId: number) => {
  const { data } = await instance.post<ScrapResponseType>(`/folders/${contentId}`, { name, type }, {});

  return data;
};

/** 2023/10/23 - 스크랩 POST 요청 - by jh */
export const postScrap = async (name: string, contentId: number) => {
  const { data } = await instance.post<ScrapResponseType>('/scrap', { name, contentId });
